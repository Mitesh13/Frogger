...
function initObstacles()
{
    ...
    {
        let x = i * 350
        carsArray.push(new Obstacle(x,canvas1.height - grid*2 - 20,grid*2,grid,1,'car'))
    }
    //lane2
    for(let i=0;i<2;i++)
    {
        let x = i * 300
        carsArray.push(new Obstacle(x,canvas1.height - grid*3 - 20,grid*2,grid,-2,'car'))
    }
    //lane3
    for(let i=0;i<2;i++)
    {
        let x = i * 400
        carsArray.push(new Obstacle(x,canvas1.height - grid*4 - 20,grid*2,grid,2,'car'))
    }
    //lane4
    for(let i=0;i<2;i++)
    {
        let x = i * 400
        logsArray.push(new Obstacle(x,canvas1.height - grid*5 - 20,grid*2,grid,-2,'log'))
    }
    //lane5
    for(let i=0;i<3;i++)
    {
        let x = i * 200
        logsArray.push(new Obstacle(x,canvas1.height - grid*6 - 20,grid,grid,1,'turtle'))
    }
}
...
function handleObstacles()
{
    ...
    //collision with logs and turtles
